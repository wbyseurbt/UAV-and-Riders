...
本文档详细描述了 `UAV-and-Riders` 项目的仿真逻辑、角色行为、协同机制及强化学习设计。文档内容基于最新代码实现（包含**强制直送逻辑**、**起飞惩罚**、**基准测试**等）。
...
*   **移动方式**：
    ...
    *   速度：需累积“移动缓冲”才能迈出一步（模拟速度差异，通常为 0.5）。

*   **行为逻辑**：
    1.  **接单（第一公里）**：
        *   系统生成订单后（0.1 概率/步），分配给距离商店最近的空闲骑手。
        *   骑手状态转为 `BUSY`，步行前往商店取货。
        *   到店后，订单状态变为 `PICKED_BY_R1`。
    2.  **决策（中途）**：
        *   **强制直送（Hard Constraint）**：如果“离骑手最近的站点”与“离订单终点最近的站点”是同一个（说明无需跨站中转），**强制**骑手直接前往终点送货，无视智能体动作。
        *   **策略控制**：若非上述情况，RL 策略决定骑手是“直送终点”还是“前往某个站点交接”。
        *   **随机强制中转（Soft Constraint）**：训练时以一定概率 (`force_prob`) 强制骑手去最近站点，但前提是：直送距离 > 2倍去站距离（避免绕路）。
    3.  **派送（最后一公里）**：
...
*   **行为逻辑**：
    1.  **起飞条件**：
        ...
        *   **起飞惩罚**：每次起飞扣除 **0.05** 分，迫使智能体只在必要时（如订单收益 > 起飞成本）才发射。
    2.  **平衡策略**：
        *   **供需匹配**：只有当源站“有余力”且目标站“缺车”时，才允许发射空机支援。
        *   **平衡奖励**：空机调度奖励降低为 **0.05**，仅用于抵消起飞惩罚，避免刷分。
...
*   **调度逻辑**：
    ...
    *   **兜底机制**：如果订单在缓冲池积压超过 **20 步**，自动转为“待派送”，呼叫骑手直接送走（转人工）。
...
> **注**：如果在步骤 2 骑手选择直送（或触发强制直送逻辑），则直接跳到步骤 7。
...
| 参数名 | 值 | 说明 |
| :--- | :--- | :--- |
| `world_grid_size` | 20x20 | 地图尺寸 |
| `n_riders` | 30 | 骑手总数 |
| `n_stations` | 5 | 站点数量 |
| `n_shops` | 8 | 商店数量 |
| `uav_speed` | 2.5 | 无人机速度 |
| `rider_speed` | 0.5 | 骑手速度 |
| `uav_capacity` | 5 | 无人机单次载单上限 |
| `force_station_prob` | 0.5 | 训练时随机强制交接概率 |
| `order_gen_rate` | 0.1 | 订单生成概率/步 (硬编码) |
...
### 6.1 渲染回放
使用 `scripts/sb3/render_rollout.py` 可视化模型表现，并查看详细统计：
*   **Rider Only**：纯骑手直送的订单数及平均时间。
*   **UAV Assisted**：无人机协助的订单数及平均时间。

### 6.2 基准测试 (Baseline)
使用 `run_baseline.py` 运行纯骑手贪婪策略（Rider-Only Greedy），作为性能对比基准。该模式下禁止无人机起飞，骑手全程直送。
