...
class TorchVecEnv(VecEnv):
    ...
    # ------------------------------------------------------------------
    # GPU-native step (no CPU transfer)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def step_tensor(self, actions: torch.Tensor):
        """Step returning GPU tensors directly — no numpy conversion.
        ...
        dones : (B,) bool tensor
        terminal_obs : (B, obs_dim) tensor or None (only when any env is done)
        trunc_no_term : (B,) bool tensor (truncated but NOT terminated)
        ep_info : tuple(np.ndarray, np.ndarray, np.ndarray) or None
            (done_indices, ep_rewards, ep_lengths) — transferred to CPU
        """
        obs, rewards, terminated, truncated, dones = self._step_impl(actions)
